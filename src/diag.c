 ...
 * \param msg The error msg plus a variadic list (printf-like) of arguments
 */
void fatal_compiler_error (compiler_error_t err, int errno, const char *msg, ...) {
  if (err >= NUM_COMPILER_ERRORS) return;

  fprintf(stderr, "Fatal Compiler Error (%s) -- ", ((errno != 0) ? compiler_err_msgs[err] : strerror(errno)));
